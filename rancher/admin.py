...
class EnvValueMixin(object):
    def _value(self, obj):
        if not obj or not obj.value:
            ...
            return mark_safe(
                "<pre style='white-space:pre-wrap'>{}</pre>".format(obj.value)
            )
        else:
            return "******"
...
class ContainerImageLinkMixin(object):
    image_change_url_name = "admin:{}_{}_change".format(
        models.ContainerImage._meta.app_label, models.ContainerImage._meta.model_name
    )
...
class ClusterLinkMixin(object):
    cluster_change_url_name = "admin:{}_{}_change".format(
        models.Cluster._meta.app_label, models.Cluster._meta.model_name
    )
...
class ProjectLinkMixin(object):
    project_change_url_name = "admin:{}_{}_change".format(
        models.Project._meta.app_label, models.Project._meta.model_name
    )
    ...
    def _project(self, obj):
        if not obj:
            ...
            if project:
                ...
                if "/rancher/project/" in self.request.path and self.request.META.get(
                    "QUERY_STRING"
                ):
                    url = "{}?{}".format(url, self.request.META.get("QUERY_STRING"))
                return mark_safe(
                    "<span style='white-space:nowrap;'><A href='{2}' target='manage_workloads'><img src='/static/img/view.jpg' width=16 height=16></A><A href='{0}' style='margin-left:5px'>{1}</A></span>".format(
                        url,
                        "{}({})".format(project.name, project.projectid)
                        if project.name
    ...
    def _projectid(self, obj):
        if not obj:
            ...
            if project:
                return mark_safe(
                    "{0}<A href='{1}' style='margin-left:20px' target='manage_workloads'><img src='/static/img/view.jpg' width=16 height=16></A>".format(
                        "{}({})".format(project.name, project.projectid)
                        if project.name
                        else project.projectid,
                        project.managementurl,
...
class NamespaceLinkMixin(object):
    namespace_change_url_name = "admin:{}_{}_change".format(
        models.Namespace._meta.app_label, models.Namespace._meta.model_name
    )
...
class VolumeLinkMixin(object):
    volume_change_url_name = "admin:{}_{}_change".format(
        models.PersistentVolume._meta.app_label,
        models.PersistentVolume._meta.model_name,
    )
...
class ContainersLinkMixin(object):
    containers_url_name = "admin:{}_{}_changelist".format(
        models.Container._meta.app_label, models.Container._meta.model_name
    )
    _containers_url = None

    container_url_name = "admin:{}_{}_change".format(
        models.Container._meta.app_label, models.Container._meta.model_name
    )

    containerlogs_url_name = "admin:{}_{}_changelist".format(
        models.ContainerLog._meta.app_label, models.ContainerLog._meta.model_name
    )
    ...
    def containers_url(self):
        if not ContainersLinkMixin._containers_url:
            ContainersLinkMixin._containers_url = reverse(
                ContainersLinkMixin.containers_url_name, args=[]
            )
    ...
    def containerlogs_url(self):
        if not ContainersLinkMixin._containerlogs_url:
            ContainersLinkMixin._containerlogs_url = reverse(
                ContainersLinkMixin.containerlogs_url_name, args=[]
            )
    ...
    def _containers(self, obj):
        if not obj:
            ...
            return mark_safe(
                "<A href='{0}?workload__id__exact={1}'>All</A>".format(url, obj.id)
            )
        elif len(obj.latest_containers) == 1:
            change_url = reverse(
                self.container_url_name, args=[obj.latest_containers[0][0]]
            )

            logs_link = ""
            if obj.latest_containers[0][2]:
                logs_link = "<A href='{0}?container__id__exact={1}'><img src='/static/img/info.png' width=16 height=16></A>".format(
                    self.containerlogs_url, obj.latest_containers[0][0]
                )

            if (
                obj.latest_containers[0][2] & models.Workload.ERROR
                ...
                obj.latest_containers[0][2] & models.Workload.WARNING
                == models.Workload.WARNING
            ):
            ...
            if logs_link:
                return mark_safe(
                    "<span style='white-space:nowrap'>{3}<A href='{0}' style='margin-left:5px'>Latest</A><A style='margin-left:5px' href='{1}?workload__id__exact={2}'>All</A></span>".format(
                        change_url, self.containers_url, obj.id, logs_link
                    )
                )
            else:
                return mark_safe(
                    "<span style='white-space:nowrap'><A href='{0}'>Latest</A><A style='margin-left:5px' href='{1}?workload__id__exact={2}'>All</A></span>".format(
            ...
            logs_link = ""
            if level:
                logs_link = "<A href='{0}?container__id__in={1}'><img src='/static/img/info.png' width=16 height=16></A>".format(
                    self.containerlogs_url, containerids
                )
            ...
            if logs_link:
                return mark_safe(
                    "<span style='white-space:nowrap'>{3}<A href='{0}?id__in={2}' style='margin-left:5px'>Latest</A><A style='margin-left:5px' href='{0}?workload__id__exact={1}'>All</A></span>".format(
                        self.containers_url, obj.id, containerids, logs_link
                    )
                )
            else:
                return mark_safe(
                    "<span style='white-space:nowrap'><A href='{0}?id__in={2}'>Latest</A><A style='margin-left:5px' href='{0}?workload__id__exact={1}'>All</A></span>".format(
...
class ContainerLinkMixin(object):
    ...
    def _container(self, obj):
        if not obj:
            ...
            return mark_safe(
                "<A href='{}?container__id__exact={}'>{} ({})</A>".format(
                    url,
                    obj.container.workload.name,
                    obj.container.containerid,
                    timezone.localtime(obj.container.started).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                    if obj.container.started
                    else "",
                )
            )
...
class LogsLinkMixin(object):
    logs_url_name = "admin:{}_{}_changelist".format(
        models.ContainerLog._meta.app_label, models.ContainerLog._meta.model_name
    )
    ...
    def _logs(self, obj):
        if not obj or not obj.log:
            ...
            if obj.warning and obj.error:
                return mark_safe(
                    "<A href='{0}?container__id__exact={1}'>Logs</A><A style='margin-left:5px' href='{0}?container__id__exact={1}&level={2}'>Errors</A><A style='margin-left:5px' href='{0}?container__id__exact={1}&level={3}'>Warnings</A>".format(
                        url,
                        obj.id,
                        models.ContainerLog.ERROR,
                        models.ContainerLog.WARNING,
                ...
                return mark_safe(
                    "<A href='{0}?container__id__exact={1}'>Logs</A><A style='margin-left:5px' href='{0}?container__id__exact={1}&level={2}'>Warnings</A>".format(
                        url, obj.id, models.ContainerLog.WARNING
                    )
                )
            elif obj.error:
                return mark_safe(
                    "<A href='{0}?container__id__exact={1}'>Logs</A><A style='margin-left:5px' href='{0}?container__id__exact={1}&level={2}'>Errors</A>".format(
                        url, obj.id, models.ContainerLog.ERROR
                    )
                )
            else:
                return mark_safe(
                    "<A href='{0}?container__id__exact={1}'>Logs</A>".format(
                        url, obj.id
                    )
                )
...
class WorkloadsLinkMixin(object):
    workloads_url_name = "admin:{}_{}_changelist".format(
        models.Workload._meta.app_label, models.Workload._meta.model_name
    )
        ...
        if not obj:
            ...
            if workloads:
                ...
                if obj.__class__ == models.Cluster:
                    return mark_safe(
                        "<A href='{}?cluster__id__exact={}'>{}</A>".format(
                            url, obj.id, workloads
                        )
                    )
                elif obj.__class__ == models.Project:
                    return mark_safe(
                        "<A href='{}?project__id__exact={}'>{}</A>".format(
                            url, obj.id, workloads
                        )
                    )
                elif obj.__class__ == models.Namespace:
                    return mark_safe(
                        "<A href='{}?namespace__id__exact={}'>{}</A>".format(
                            url, obj.id, workloads
                        )
                    )
                elif obj.__class__ == models.ContainerImage:
                    return mark_safe(
                        "<A href='{}?containerimage__id__exact={}'>{}</A>".format(
                            url, obj.id, workloads
                        )
                    )
                else:
    ...
    def _active_workloads(self, obj):
        if not obj:
            ...
            if workloads:
                ...
                if obj.__class__ == models.Cluster:
                    return mark_safe(
                        "<A href='{}?cluster__id__exact={}&deleted__isnull=True'>{}</A>".format(
                            url, obj.id, workloads
                        )
                    )
                elif obj.__class__ == models.Project:
                    return mark_safe(
                        "<A href='{}?project__id__exact={}&deleted__isnull=True'>{}</A>".format(
                            url, obj.id, workloads
                        )
                    )
                elif obj.__class__ == models.Namespace:
                    return mark_safe(
                        "<A href='{}?namespace__id__exact={}&deleted__isnull=True'>{}</A>".format(
                            url, obj.id, workloads
                        )
                    )
                else:
    ...
    def _deleted_workloads(self, obj):
        if not obj:
            ...
            if workloads:
                ...
                if obj.__class__ == models.Cluster:
                    return mark_safe(
                        "<A href='{}?cluster__id__exact={}&deleted__isnull=False'>{}</A>".format(
                            url, obj.id, workloads
                        )
                    )
                elif obj.__class__ == models.Project:
                    return mark_safe(
                        "<A href='{}?project__id__exact={}&deleted__isnull=False'>{}</A>".format(
                            url, obj.id, workloads
                        )
                    )
                elif obj.__class__ == models.Namespace:
                    return mark_safe(
                        "<A href='{}?namespace__id__exact={}&deleted__isnull=False'>{}</A>".format(
                            url, obj.id, workloads
                        )
                    )
                else:
...
class WorkloadLinkMixin(object):
    workload_change_url_name = "admin:{}_{}_change".format(
        models.Workload._meta.app_label, models.Workload._meta.model_name
    )
    ...
    def _workload(self, obj):
        if not obj:
            ...
            if workload.is_deleted or workload.added_by_log:
                return mark_safe(
                    "<A href='{}'>{}({})</A>".format(url, workload.name, workload.kind)
                )
            else:
                return mark_safe(
                    "<A href='{3}' target='manage_workload'><img src='/static/img/view.jpg' width=16 height=16></A><A href='{4}' target='manage_workload' style='margin-left:5px'><img src='/static/img/setting.jpg' width=16 height=16></A><A href='{0}' style='margin-left:5px'>{1}({2})</A>".format(
                        url,
                        workload.name,
                        workload.kind,
                        workload.viewurl,
                        workload.managementurl,
                    )
                )
    ...
    def _manage(self, obj):
        if not obj:
            ...
            if workload.is_deleted or workload.added_by_log:
                ...
                return mark_safe(
                    "<A href='{}' target='manage_workload'><img src='/static/img/view.jpg' width=16 height=16></A><A href='{}' target='manage_workload' style='margin-left:20px'><img src='/static/img/setting.jpg' width=16 height=16></A>".format(
                        workload.viewurl, workload.managementurl
                    )
                )
    ...
    def _name(self, workload):
        if not workload:
            ...
            return mark_safe(
                "{}<A href='{}' style='margin-left:20px' target='manage_workload'><img src='/static/img/view.jpg' width=16 height=16></A><A href='{}' target='manage_workload' style='margin-left:20px'><img src='/static/img/setting.jpg' width=16 height=16></A>".format(
                    workload.name, workload.viewurl, workload.managementurl
                )
            )
    ...
    def _name_with_link(self, obj):
        if not obj:
            ...
            if workload.is_deleted or workload.added_by_log:
                ...
                return mark_safe(
                    "<A href='{2}' target='manage_workload'><img src='/static/img/view.jpg' width=16 height=16></A><A href='{3}' target='manage_workload' style='margin-left:5px'><img src='/static/img/setting.jpg' width=16 height=16></A><A href='{0}' style='margin-left:5px'>{1}</A>".format(
                        url, workload.name, workload.viewurl, workload.managementurl
                    )
                )
    ...
    def enforce_refresh(self, request, queryset):
        for cluster in queryset:
            try:
                ...
                self.message_user(
                    request,
                    "The process to harvest all configuration files of the cluster({}) has been scheduled.".format(
                        cluster
                    ),
                )
                ...
                self.message_user(
                    request,
                    "Failed to schedule the process to harvest all configuration files of the cluster({}).{}".format(
                        cluster, str(ex)
                    ),
    ...
    def refresh(self, request, queryset):
        for cluster in queryset:
            try:
                ...
                self.message_user(
                    request,
                    "The process to harvest the changed configuration files of the cluster({}) has been scheduled.".format(
                        cluster
                    ),
                )
            except Exception as ex:
                self.message_user(
                    request,
                    "Failed to schedule the process to harvest the changed configuration files of the cluster({}).{}".format(
                        cluster, str(ex)
                    ),
                    level=messages.ERROR,
                )
...
class DeletedMixin(object):
    def _deleted(self, obj):
        if not obj:
            ...
            return mark_safe(
                "<img src='/static/admin/img/icon-yes.svg'> {}".format(
                    timezone.localtime(obj.deleted).strftime("%Y-%m-%d %H:%M:%S")
                )
            )
        else:
...
class WorkloadInline4Namespace(
    DeletedMixin, WorkloadLinkMixin, DatetimeMixin, admin.TabularInline
):
...
class SecretItemInline(
    RequestMixin,
    SecretPermissionMixin,
    DatetimeMixin,
    EnvValueMixin,
    admin.TabularInline,
):
    ...
    is_developer = staticmethod(
        lambda user, obj: models.WorkloadEnv.objects.filter(
            secret=obj.secret,
            workload__containerimage__imagefamily__contacts__contains=[user.email],
        )
        .only("id")
        .exists()
        if obj
...
class SecretAdmin(
    ...
    def _references(self, obj):
        if not obj:
            return ""
        result = None
        for item in models.WorkloadEnv.objects.filter(secret=obj).order_by(
            "workload", "name"
        ):
            ...
            if result:
                ...
                result = "{3}<br><A href={0}>{1}.{2}<>".format(
                    url, item.workload.item.name, result
                )
...
class PersistentVolumeAdmin(
    LookupAllowedMixin, DeletedMixin, ClusterLinkMixin, DatetimeMixin, admin.ModelAdmin
):
    list_display = (
        "name",
        "_cluster",
...
class WorkloadEnvInline(
    ...
    is_developer = staticmethod(
        lambda user, obj: user.email
        in (obj.workload.containerimage.imagefamily.contacts or [])
        if obj
        else False
    )

    configmap_change_url_name = "admin:{}_{}_change".format(
        models.ConfigMap._meta.app_label, models.ConfigMap._meta.model_name
    )
    ...
    secret_change_url_name = "admin:{}_{}_change".format(
        models.Secret._meta.app_label, models.Secret._meta.model_name
    )
...
class VulnerabilitiesMixin(object):
    vulns_url_name = "admin:{}_{}_changelist".format(
        models.Vulnerability._meta.app_label, models.Vulnerability._meta.model_name
    )
    get_os = None
    ...
    def _vulnerabilities(self, obj):
        if not obj:
            ...
            if obj.highs:
                if result:
                    result = "{}<span style='margin-left:5px'>High:{}</span>".format(
                        result, obj.highs
                    )
                else:
            ...
            if obj.mediums:
                if result:
                    result = "{}<span style='margin-left:5px'>Medium:{}</span>".format(
                        result, obj.mediums
                    )
                else:
            ...
            if obj.lows:
                if result:
                    result = "{}<span style='margin-left:5px'>Low:{}</span>".format(
                        result, obj.lows
                    )
                else:
            ...
            if obj.unknowns:
                if result:
                    result = "{}<span style='margin-left:5px'>Unknown:{}</span>".format(
                        result, obj.unknowns
                    )
                else:
...
class ImagesMixin(object):
    images_url_name = "admin:{}_{}_changelist".format(
        models.ContainerImage._meta.app_label, models.ContainerImage._meta.model_name
    )
    get_os = None
    ...
    def _images(self, obj):
        if not obj:
            ...
            if not obj or not obj.images:
                ...
                url = "{}?os={}".format(
                    reverse(self.images_url_name, args=None), obj.id
                )
...
class OperatingSystemInline(
    RequestMixin, ImagesMixin, VulnerabilitiesMixin, admin.TabularInline
):
    model = models.Vulnerability.oss.through
    readonly_fields = (
        "name",
        ...
        if not obj:
            ...
            if obj.scan_status < 0:
                ...
                if obj.highs:
                    if result:
                        result = (
                            "{}<span style='margin-left:5px'>High:{}</span>".format(
                                result, obj.highs
                            )
                        )
                    else:
                ...
                if obj.mediums:
                    if result:
                        result = (
                            "{}<span style='margin-left:5px'>Medium:{}</span>".format(
                                result, obj.mediums
                            )
                        )
                    else:
                ...
                if obj.unknowns:
                    if result:
                        result = (
                            "{}<span style='margin-left:5px'>Unknown:{}</span>".format(
                                result, obj.unknowns
                            )
                        )
                    else:
...
class ContainerImageInline(
    RequestMixin, ScanSummaryMixin, WorkloadsLinkMixin, admin.TabularInline
):
    ...
    def get_queryset(self, request):
        ...
        qs = qs.select_related("containerimage").defer(
            "containerimage__scan_result",
            "containerimage__scan_message",
            "containerimage__vulnerabilities",
        )
        return qs
...
class OperatingSystemAdmin(
    RequestMixin, ImagesMixin, VulnerabilitiesMixin, admin.ModelAdmin
):
    list_display = (
        "name",
        "version",
...
class VulnerabilityAdmin(RequestMixin, admin.ModelAdmin):
    ...
    list_url_name = "{}_{}_changelist".format(
        models.Vulnerability._meta.app_label, models.Vulnerability._meta.model_name
    )
    ...
    def _scan_result(self, obj):
        if not obj:
            ...
            return format_html(
                """<A href="javascript:void" onclick="django.jQuery('#id_image_{0}').toggle();django.jQuery(this).html((django.jQuery(this).html() == 'Show')?'Hide':'Show')">Show</A>
<pre id="id_image_{0}" style="display:none;white-space:break-spaces">
{1}
</pre>""",
                obj.id,
                json.dumps(obj.scan_result, indent=4),
            )
    ...
    def _description(self, obj):
        if not obj:
            ...
            return mark_safe(
                "<pre style='white-space:break-spaces'>{}</pre>".format(obj.description)
            )
...
class VulnerabilityInline4Image(RequestMixin, admin.TabularInline):
    ...
    ordering = (
        ...
        "vulnerability__vulnerabilityid",
        "vulnerability__pkgname",
    )
    classes = ["collapse"]

    def get_queryset(self, request):
        qs = super().get_queryset(request)
        qs = qs.select_related("vulnerability").defer(
            "vulnerability__scan_result",
...
class ContainerImageAdmin(
    RequestMixin, DatetimeMixin, ScanSummaryMixin, WorkloadsLinkMixin, admin.ModelAdmin
):
    list_display = (
        "imageid",
        "_account",
        ...
        for image in queryset:
            try:
                ...
                if image.scan_status in (image.SCAN_FAILED, image.PARSE_FAILED):
                    self.message_user(
                        request,
                        "Failed to scan the image '{}'.{}".format(
                            image.imageid, image.scan_message
                        ),
                        level=messages.ERROR,
                    )
                else:
                    self.message_user(
                        request, "Succeed to scan the image '{}'".format(image.imageid)
                    )
            except Exception as ex:
                self.message_user(
                    request,
                    "Failed to scan the image '{}'. {}".format(image.imageid, str(ex)),
                    level=messages.ERROR,
                )
...
class ResourceIdMixin(object):
    def _resource_id(self, obj):
        if not obj or not obj.resource_id:
            ...
            obj.resource_type >= models.EnvScanModule.CREDENTIAL
            and obj.resource_type < models.EnvScanModule.CREDENTIAL + 10
        ):
            return "******"
        else:
            return obj.resource_id
...
class WorkloadAdmin(
    ...
    readonly_fields = (
        (
            ...
            "suspend",
            "added_by_log",
            "_resource_dependent_tree",
            "_workload_dependent_tree",
        )
        if settings.ENABLE_ADDED_BY_CONTAINERLOG
        else (
            "_name",
    ...
    inlines = [
        WorkloadResourceInline,
        WorkloadListeningInline,
        WorkloadEnvInline,
        WorkloadVolumeInline,
    ]
    ...
    is_developer = staticmethod(
        lambda user, obj: user.email in (obj.containerimage.imagefamily.contacts or [])
        if obj
        else False
    )
    ...
    def _format_resource_tree(self, root_workload, tree, indent=0):
        ...
        for dependent_type, dependent_type_name, type_dependents in tree[3]:
            ...
            for dependent_pk, dependent_display, sub_dep_trees in type_dependents:
                if (
                    dependent_type >= models.EnvScanModule.CREDENTIAL
                    and dependent_type < models.EnvScanModule.CREDENTIAL + 10
                    and not self.secretpermission_granted(root_workload)
                ):
                    dependent_display = "******"
                ...
                if sub_dep_trees:
                    ...
                    sub_tree_html = "".join(
                        self._format_resource_tree(root_workload, sub_tree, indent + 48)
                        for sub_tree in sub_dep_trees
                    )
                    ...
                    type_tree_html = (
                        "{0}<div style='margin-left:{2}px'>{1}</div>".format(
        ...
        if indent > 0:
            if tree[3]:
                ...
                if tree[2]:
                    tree_html = "<div style='margin-left:{1}px'>{0}</div>".format(
                        workload_name, indent
                    )
                else:
                    tree_html = "<div style='margin-left:{1}px;text-decoration-line: line-through;color:red'>{0}</div>".format(
                        workload_name, indent
                    )
...
""".format(
                                self._format_resource_tree(obj, tree)
                            )
                        )
                else:
                    return ""
    ...
    def _format_workload_tree(self, root_workload, tree, dependents=None, indent=0):
        ...
        if dependents:
            ...
            dependents_html = "<img src='/static/img/link.svg' onclick=\"$('#{1}').toggle()\" style='margin-left:5px'><div id='{1}' style='display:none'>{0}</div>".format(
                "".join(
                    "<div style='margin-left:32px;color:blue'>- {} : {}</div>".format(
                        dep[1],
                        dep[3]
                        if self.secretpermission_granted(root_workload)
                        else "******",
                    )
        ...
        if tree[3]:
            ...
            if tree[2]:
                tree_html = (
                    "<div style='margin-left:{1}px;'><span>{0}</span>{2}</div>".format(
                        workload_name, indent, dependents_html
                    )
                )
            else:
                tree_html = "<div style='margin-left:{1}px;'><span style='text-decoration-line: line-through;color:red'>{0}</span>{2}</div>".format(
                    workload_name, indent, dependents_html
                )
...
""".format(
                                self._format_workload_tree(obj, tree)
                            )
                        )
                else:
                    return ""
    ...
    def scan_dependency(self, request, queryset):
        for workload in queryset:
            try:
                ...
                self.message_user(
                    request,
                    "Succeed to scan the dependency of the workload '{}'".format(
                        workload
                    ),
                )
            except Exception as ex:
                self.message_user(
                    request,
                    "Failed to scan the dependency of the workload '{}'. {}".format(
                        workload, str(ex)
                    ),
                    level=messages.ERROR,
                )
    ...
    def populate_resource_tree(self, request, queryset):
        for workload in queryset:
            try:
                ...
                self.message_user(
                    request,
                    "Succeed to poplate the resource dependent tree for the workload '{}'".format(
                        workload
                    ),
                )
            except Exception as ex:
                self.message_user(
                    request,
                    "Failed to populate the resource dependent tree for the workload '{}'. {}".format(
                        workload, str(ex)
                    ),
                    level=messages.ERROR,
                )
    ...
    def populate_workload_tree(self, request, queryset):
        for workload in queryset:
            try:
                ...
                self.message_user(
                    request,
                    "Succeed to poplate the workload dependent tree for the workload '{}'".format(
                        workload
                    ),
                )
            except Exception as ex:
                self.message_user(
                    request,
                    "Failed to populate the workload dependent tree for the workload '{}'. {}".format(
                        workload, str(ex)
                    ),
                    level=messages.ERROR,
                )
...
class ContainerAdmin(
    ...
    containers_url_name = "{}_{}_changelist".format(
        models.Container._meta.app_label, models.Container._meta.model_name
    )
    ...
    is_developer = staticmethod(
        lambda user, obj: user.email in obj.workload.containerimage.imagefamily.contacts
        if (obj and obj.workload.containerimage.imagefamily.contacts)
        else False
    )
    ...
    def get_queryset(self, request):
        ...
        for key in request.GET.keys():
            if key == "workload" or key.startswith("workload__"):
                ...
                key in ("id", "pk") or key.startswith("id__") or key.startswith("pk__")
            ):
                query_allowed = True
                break
    ...
    def _container_terminated(self, obj):
        if not obj or not obj.container_terminated:
            ...
            return timezone.localtime(obj.container_terminated).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
    ...
    def _container_started(self, obj):
        if not obj or not obj.container_started:
            ...
            return timezone.localtime(obj.container_started).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
    ...
    def _container_created(self, obj):
        if not obj or not obj.container_created:
            ...
            return timezone.localtime(obj.container_created).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
...
class ContainerLogAdmin(LookupAllowedMixin, ContainerLinkMixin, admin.ModelAdmin):
    ...
    containerlogs_url_name = "{}_{}_changelist".format(
        models.ContainerLog._meta.app_label, models.ContainerLog._meta.model_name
    )
    ...
    def _message(self, obj):
        if not obj or not obj.message:
            ...
            return mark_safe(
                "<pre style='white-space:normal'>{}</pre>".format(obj.message)
            )
...
class EnvScanModuleAdmin(DatetimeMixin, admin.ModelAdmin):
    ...
    list_url_name = "{}_{}_changelist".format(
        models.EnvScanModule._meta.app_label, models.EnvScanModule._meta.model_name
    )
...
class ContainerImageFamilyAdmin(
    RequestUserMixin, SecretPermissionMixin, DatetimeMixin, admin.ModelAdmin
):
    list_display = (
        "account",
        "name",
    ...
    list_url_name = "{}_{}_changelist".format(
        models.ContainerImageFamily._meta.app_label,
        models.ContainerImageFamily._meta.model_name,
    )
...
class WorkloadResourceAdmin(
    ...
    def get_queryset(self, request):
        qs = super().get_queryset(request)
        qs = qs.select_related("workload", "imagefamily").only(
            "resource_type",
            "config_items",
            "resource_id",
            "config_source",
    ...
    def _properties(self, obj):
        print(
            "{} in {}   {}".format(
                ...
                obj.workload.containerimage.imagefamily.contacts,
                self.secretpermission_granted(obj),
            )
        )
