...
def host_status_export(fileobj, host_status):
    ...
    with xlsxwriter.Workbook(
        ...
        date_summary = ", ".join(
            (
                x.isoformat()
                for x in host_status.values_list("date", flat=True)
                .order_by("date")
                .distinct()
            )
        )
        ...
        sheet.write_row(
            2,
            0,
            ("Host name", "Scan range", "Category", "Status", "Plugin", "Output"),
            bold,
        )
        row = 3
        for status in host_status:
            ...
            for cat in ("monitor", "vulnerability", "backup", "patching"):
                ...
                sheet.write_row(
                    row, 0, (host, scan_range, cat, stat, plug_name, output)
                )
                if url:
            ...
            os = (
                status.vulnerability_info["os"]
                if status.vulnerability_info and "os" in status.vulnerability_info
                else "Unknown"
            )
            ...
            num_critical = (
                status.vulnerability_info["num_critical"]
                if status.vulnerability_info
                and "num_critical" in status.vulnerability_info
                else "Unknown"
            )
            sheet.write_row(
                row, 0, (host, scan_range, "num_critical", "-", "-", num_critical)
            )
            row += 1

            num_high = (
                status.vulnerability_info["num_high"]
                if status.vulnerability_info and "num_high" in status.vulnerability_info
                else "Unknown"
            )
