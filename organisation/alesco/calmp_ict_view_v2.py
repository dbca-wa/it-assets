...
FOREIGN_TABLE_FIELDS = (
    ...
    (
        ...
        if val and val != ALESCO_DATE_MAX
        else None,
    ),
    (
        "occup_term_date",
        ...
        if manager_employee_no:
            try:
                ...
                if user and manager.pk == user.pk:
                    logger.debug(
                        "The manager of the user({0}.{1}) is himself/herself".format(
                            get_firstname_from_alesco(jobs),
                            get_surname_from_alesco(jobs),
                        )
                    )
                else:
                    return manager
                ...
                logger.error(
                    "The manager({2}) of the user({0}.{1}) doesn't exist".format(
                        get_firstname_from_alesco(jobs),
                        get_surname_from_alesco(jobs),
                        manager_employee_no,
                    )
...
def get_term_datetime_from_alesco(jobs):
    ...
    term_date = (
        TZ.localize(datetime.strptime(term_date, "%Y-%m-%d")) if term_date else None
    )
...
def get_location_from_alesco(jobs):
    ...
    if location:
        ...
        if loc:
            ...
            logger.error(
                "The location({2}) of the user({0}.{1}) doesn't exist".format(
                    get_firstname_from_alesco(jobs),
                    get_surname_from_alesco(jobs),
                    location,
                )
    ...
    if manager:
        if manager != user.parent:
            if manager in user.get_descendants():
                logger.info(
                    "Removing manager relationship from {}, should be fixed next cycle".format(
                        manager.email
                    )
                )
                manager.parent = None
            ...
            logger.info(
                "Updating manager for {} from {} to {}".format(
                    user.email,
                    user.parent.email if user.parent else None,
...
def update_user_data_from_alesco(
    ...
    if data:
        if data != getattr(user, property_name):
            logger.info(
                "Updating {} for {} from {} to {}".format(
                    property_name, user.email, getattr(user, property_name), data
                )
            )
...
def update_title_from_alesco(user, update_fields=[], commit=True):
    update_user_data_from_alesco(
        user,
        "title",
        get_occup_pos_title_from_alesco,
...
def print_records(file_name=None, f_filter=None):
    ...
    try:
        for eid, jobs in employee_iter:
            ...
            _writer(
                "employee_id={}, manager employee id={}, paypoint={}, location={}, location desc={}, job term date={}".format(
                    eid,
            ...
            for job in jobs:
                _writer(
                    "    score={} job_term_date={}, emp_status={}, surname={}, given_name={}, title={}, gender={}, paypoint={}, location={}, location_desc={}, manager emplyee no={}".format(
