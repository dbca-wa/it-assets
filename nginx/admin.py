...
class WebServerAdmin(admin.ModelAdmin):
    ...
    app_change_url_name = "admin:{}_{}_change".format(
        models.WebApp._meta.app_label, models.WebApp._meta.model_name
    )
    ...
    def _apps(self, obj):
        if not obj:
            ...
            apps = set()
            for location_server in models.WebAppLocationServer.objects.filter(
                server=obj
            ):
            ...
            return mark_safe(
                "<pre>{}</pre>".format(
                    "\n".join(
                        "<A href='{}'>{}</A>".format(
                            reverse(self.app_change_url_name, args=(app.id,)), app.name
                        )
                        for app in apps
                    )
                )
...
class ConfigureTxtMixin(object):
    def _configure_txt(self, obj):
        if not obj or not obj.configure_txt:
            ...
            return format_html(
                """<A href="javascript:void" onclick="django.jQuery('#id_{0}_{1}').toggle();django.jQuery(this).html((django.jQuery(this).html() == 'Show')?'Hide':'Show')">Show</A>
...
class WebAppLocationMixin(object):
    workload_change_url_name = "admin:{}_{}_change".format(
        Workload._meta.app_label, Workload._meta.model_name
    )
        ...
        if not obj:
            ...
            for server in obj.servers.all():
                if server.rancher_workload:
                    url = reverse(
                        self.workload_change_url_name,
                        args=(server.rancher_workload.id,),
                    )
                    if result:
                        result = "{}\n<A href='{}'>{}://{}:{}{}</A><A href='{}' style='margin-left:50px' target='manage_workload'><img src='/static/img/view.jpg' width=16 height=16></A><A href='{}' target='manage_workload' style='margin-left:20px'><img src='/static/img/setting.jpg' width=16 height=16></A>".format(
                            result,
                            url,
                            obj.get_forward_protocol_display(),
                            server.server.name,
                            server.port,
                            obj.forward_path or "",
                            server.rancher_workload.viewurl,
                            server.rancher_workload.managementurl,
                        )
                    else:
                        result = "<A href='{}'>{}://{}:{}{}</A><A href='{}' style='margin-left:50px' target='manage_workload'><img src='/static/img/view.jpg' width=16 height=16></A><A href='{}' target='manage_workload' style='margin-left:20px'><img src='/static/img/setting.jpg' width=16 height=16></A>".format(
                            url,
                            obj.get_forward_protocol_display(),
                            server.server.name,
                            server.port,
...
class WebAppAdmin(ConfigureTxtMixin, admin.ModelAdmin):
        ...
        def queryset(self, request, queryset):
            if self.value() == "true":
                return queryset.filter(
                    Q(redirect_to__isnull=False) | Q(redirect_to_other__isnull=False)
                )
            if self.value() == "false":
                return queryset.filter(
                    redirect_to__isnull=True, redirect_to_other__isnull=True
                )
    ...
    list_filter = (
        ...
        "clientip_subnet",
        RedirectToFilter,
        ("system_alias__system", admin.RelatedOnlyFieldListFilter),
    )
    readonly_fields = (
        "name",
        "auth_domain",
    ...
    dailyreport_list_url_name = "admin:{}_{}_changelist".format(
        models.WebAppAccessDailyReport._meta.app_label,
        models.WebAppAccessDailyReport._meta.model_name,
    )
    ...
    def _daily_access_report(self, obj):
        if not obj:
            ...
            return mark_safe(
                "<A href='{0}?log_day=7d&q={1}'>7 days</A><A style='margin-left:20px' href='{0}?log_day=4w&q={1}'>4 weeks</A>".format(
                    reverse(self.dailyreport_list_url_name), obj.name
                )
            )
    ...
    def _redirect(self, obj):
        ...
        if obj.redirect_to:
            return mark_safe(
                "<A href='/admin/nginx/webapp/{0}/change/'>{1}{2}</A>".format(
                    obj.redirect_to.id, obj.redirect_to, obj.redirect_path or ""
                )
            )
...
class LogDayFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if self.value():
            return queryset.filter(
                log_starttime__gte=timezone.make_aware(
                    datetime.strptime(self.value(), "%Y-%m-%d")
                ),
                log_starttime__lt=timezone.make_aware(
                    datetime.strptime(self.value(), "%Y-%m-%d")
                )
                + timedelta(days=1),
            )
...
class HttpStatusGroupFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if not val:
            ...
            return queryset.filter(Q(http_status=0) | Q(http_status__gte=400)).exclude(
                http_status__in=(401, 403, 408, 499)
            )
...
class WebServerMixin(object):
    ...
    def _webserver(self, obj):
        if not obj:
            ...
            return mark_safe(
                "<A href='{}'>{}</A>".format(
                    reverse(self.app_change_url_name, args=(obj.webapp.id,)),
                    obj.webserver,
                )
            )
...
class WebAppAccessDailyReportAdmin(WebServerMixin, admin.ModelAdmin):
    list_display = (
        ...
        "_success_requests",
        "_error_requests",
        "_unauthorized_requests",
        "_timeout_requests",
        "_client_closed_requests",
    )
    ...
    dailylog_list_url_name = "admin:{}_{}_changelist".format(
        models.WebAppAccessDailyLog._meta.app_label,
        models.WebAppAccessDailyLog._meta.model_name,
    )
    ...
    def _requests(self, obj):
        if not obj:
            ...
            return mark_safe(
                "<A href='{}?log_day={}&q={}'>{}</A>".format(
                    reverse(self.dailylog_list_url_name),
    ...
    def _success_requests(self, obj):
        if not obj:
            ...
            return mark_safe(
                "<A href='{}?log_day={}&q={}&http_status_group=succeed'>{}</A>".format(
                    reverse(self.dailylog_list_url_name),
                    obj.log_day.strftime("%Y-%m-%d"),
                    obj.webserver,
                    obj.success_requests,
                )
            )
    ...
    def _error_requests(self, obj):
        if not obj:
            ...
            return mark_safe(
                "<A href='{}?log_day={}&q={}&http_status_group=error'>{}</A>".format(
                    reverse(self.dailylog_list_url_name),
                    obj.log_day.strftime("%Y-%m-%d"),
                    obj.webserver,
                    obj.error_requests,
    ...
    def _client_closed_requests(self, obj):
        if not obj:
            ...
            return mark_safe(
                "<A href='{}?log_day={}&q={}&http_status_group=client_closed'>{}</A>".format(
                    reverse(self.dailylog_list_url_name),
                    obj.log_day.strftime("%Y-%m-%d"),
                    obj.webserver,
                    obj.client_closed_requests,
                )
            )
    ...
    def _unauthorized_requests(self, obj):
        if not obj:
            ...
            return mark_safe(
                "<A href='{}?log_day={}&q={}&http_status_group=unauthorized'>{}</A>".format(
                    reverse(self.dailylog_list_url_name),
                    obj.log_day.strftime("%Y-%m-%d"),
                    obj.webserver,
                    obj.unauthorized_requests,
                )
            )
    ...
    def _timeout_requests(self, obj):
        if not obj:
            ...
            return mark_safe(
                "<A href='{}?log_day={}&q={}&http_status_group=timeout'>{}</A>".format(
                    reverse(self.dailylog_list_url_name),
                    obj.log_day.strftime("%Y-%m-%d"),
                    obj.webserver,
                    obj.timeout_requests,
                )
            )
...
class RequestPathMixin(object):
    location_change_url_name = "admin:{}_{}_change".format(
        models.WebAppLocation._meta.app_label, models.WebAppLocation._meta.model_name
    )
    ...
    def _request_path(self, obj):
        if not obj:
            ...
            return mark_safe(
                "<A href='{}'>{}</A>".format(
                    reverse(
                        self.location_change_url_name, args=(obj.webapplocation.id,)
                    ),
                    obj.request_path,
                )
            )
...
class WebAppAccessDailyLogAdmin(
    HttpStatusMixin,
    ResponseTimeMixin,
    WebServerMixin,
    RequestPathMixin,
    ...
    search_fields = ["webserver"]
    log_list_url_name = "admin:{}_{}_changelist".format(
        models.WebAppAccessLog._meta.app_label, models.WebAppAccessLog._meta.model_name
    )
    ...
    def _requests(self, obj):
        if not obj:
            ...
            return mark_safe(
                "<A href='{}?log_day={}&{}&request_path={}&http_status={}&{}'>{}</A>".format(
                    reverse(self.log_list_url_name),
                    obj.log_day.strftime("%Y-%m-%d"),
                    "webserver={}".format(obj.webserver)
                    if obj.webserver
                    else "webserver__isnull=True",
                    urllib.parse.quote(obj.request_path),
                    obj.http_status,
                    "path_parameters={}".format(urllib.parse.quote(obj.path_parameters))
                    if obj.path_parameters
                    else "path_parameters__isnull=True",
                    obj.requests,
                )
            )
...
class WebAppAccessLogAdmin(
    ...
    def get_urls(self):
        ...
        urls = [
            path(
                "run_log_harvesting/",
                self.run_log_harvesting,
                name="run_log_harvesting",
            ),
        ] + urls
        return urls
    ...
    def run_log_harvesting(self, request):
        try:
            ...
            self.message_user(
                request,
                "Failed to schedule the log harvesting process.{}".format(str(ex)),
