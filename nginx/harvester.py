...
def get_resource_consume_client():
    ...
    if _resource_consume_client is None:
        _resource_consume_client = ResourceConsumeClient(
            AzureBlobStorage(
                settings.NGINX_STORAGE_CONNECTION_STRING, settings.NGINX_CONTAINER
            ),
...
upstream_block_re = re.compile(
    "(\s+|^)upstream\s+(?P<name>[a-zA-Z0-9_\-]+)\s*{(?P<body>[^\}]+)}"
)
comments_re = re.compile("#[^\n]+(\n|$)")
upstream_server_re = re.compile(
    "(^|\s+|;)\s*server\s+(?P<server>[a-zA-Z0-9_\-\.]+):(?P<port>[0-9]+)"
)
clientip_subnet_block_re = re.compile(
    "(\s+|^)geo\s+\$(?P<name>[a-zA-Z0-9_\-]+)\s*{(?P<body>[^\}]+)}"
)
clientip_subnet_rule_re = re.compile(
    "^\s*if\s+\(\s*\$(?P<name>[a-zA-Z0-9_\-]+)\s*=\s*0\s*\)\s*{\s*return\s*(403|401)\s*;?\s*}\s*;?\s*$",
    re.IGNORECASE,
)
    ...
    if pre_conf:
        ...
        for m in upstream_block_re.finditer(pre_conf):
            ...
            for s_m in upstream_server_re.finditer(body):
                upstream_servers[name].append(
                    [s_m.group("server"), int(s_m.group("port"))]
                )
        logger.debug(
            """upstream servers:
{}
""".format(
                json.dumps(upstream_servers, cls=JSONEncoder, indent="    ")
            )
        )
...
{}
""".format(
                json.dumps(clientip_subnets, cls=JSONEncoder, indent="    ")
            )
        )
    ...
    def expand_include(include, prefix=""):
        ...
        if not isinstance(include_config, str):
            raise Exception(
                "The nginx include({}) is type({}),Not supported".foramt(
                    include, include_config.__class__.__name__
                )
            )
        ...
        while i >= 0:
            ...
            if striped_line.startswith("include"):
                ...
                expanded_include = expand_include(
                    nested_include, prefix="{}{}".format(prefix, subprefix)
                )
    ...
    for server, server_config in nginx_servers.items():
        ...
        if "redirect" in server_config:
            ...
            expanded_server_config = """{}
redirect {};""".format(
                expanded_server_config, server_redirect
            )
            ...
            for include in server_includes:
                ...
                if m:
                    ...
                    if clientip_subnet not in clientip_subnets:
                        raise Exception(
                            "Geo block for ({}) Not Found".format(clientip_subnet)
                        )
                    expanded_include = """{}
{}""".format(
                        expanded_include, clientip_subnets[clientip_subnet]
                    )

                expanded_server_config = """{}
{}""".format(
                    expanded_server_config, expanded_include
                )
...
{}
                    ...
                    expanded_server_config,
                    location_path,
                    os.linesep.join(location_rules),
                )
    ...
    for server in sorted(servers.keys(), key=lambda name: name.split(".", 1)[0]):
        ...
        try:
            server_alias, server_domain, server_env = parse_server(
                server, domains=domains, system_envs=system_envs
            )
            ...
            (
                ...
                server_listens,
                server_locations,
            ) = parse_server_config(server, server_config, upstream_servers)
...
{}""".format(
                    ...
                    server_env,
                    os.linesep.join(
                        "host={listen_host} port={listen_port} https={https}".format(
                            **l
                        )
                        for l in server_listens
                    ),
                    json.dumps(server_locations, cls=JSONEncoder, indent="    "),
                )
            )
        # save app
        ...
        sso_required = False
        for location in server_locations:
            if location["auth_type"] in (
                WebAppLocation.SSO_AUTH,
                WebAppLocation.SSO_AUTH_DUAL,
            ):
                sso_required = True
                break
        ...
        if sso_required:
            for location in server_locations:
                a_domain = next(
                    (t for t in sso_auth_domains if t[0] == location["auth_type"]), None
                )
                if a_domain:
        ...
        if not app.pk:
            ...
            if update_fields:
                ...
                logger.debug(
                    "Update the WebApp({}),update_fields={}".format(app, update_fields)
                )
        ...
        for listen in list(WebAppListen.objects.filter(app=app)):
            ...
            while index >= 0:
                if (
                    server_listens[index]["listen_host"] == listen.listen_host
                    and server_listens[index]["listen_port"] == listen.listen_port
                ):
            ...
            if server_listen:
                ...
                if update_fields:
                    ...
                    logger.debug(
                        "Update the WebAppListen({1}) from server({0}),update_fields={2}".format(
                            app, listen, update_fields
                        )
                    )
            else:
                ...
                listen.delete()
                logger.debug(
                    "Delete the WebAppListen({1}) from server({0})".format(app, listen)
                )
        ...
        for server_listen in server_listens:
            ...
            listen.save()
            logger.debug(
                "Create the WebAppListen({1}) from server({0})".format(app, listen)
            )
        ...
        for location in list(WebAppLocation.objects.filter(app=app)):
            ...
            while index >= 0:
                if (
                    server_locations[index]["location_type"] == location.location_type
                    and server_locations[index]["location"] == location.location
                ):
            ...
            if server_location:
                ...
                if update_fields:
                    ...
                    logger.debug(
                        "Update the WebAppLocation({1}) from server({0}),update_fields={2}".format(
                            app, location, update_fields
                        )
                    )

                if server_location["forward_servers"]:
                    save_location_forward_servers(
                        location, server_location["forward_servers"], created=False
                    )
            else:
                # not found in the current configuration,delete it
                location.delete()
                logger.debug(
                    "Delete the WebAppLocation({1}) from server({0})".format(
                        app, location
                    )
                )
        ...
        for server_location in server_locations:
            ...
            location.save()
            logger.debug(
                "Create the WebAppLocation({1}) from server({0})".format(app, location)
            )
            if server_location["forward_servers"]:
                save_location_forward_servers(
                    location, server_location["forward_servers"], created=True
                )
            ...
            try:
                ...
                if not app.pk:
                    ...
                    if update_fields:
                        ...
                        app.save(update_fields=update_fields)
                        logger.debug(
                            "Update the WebApp({}),update_fields={}".format(
                                app, update_fields
                            )
                        )
    ...
    del_objs = (
        WebApp.objects.exclude(pk__in=redirect_serverids)
        .filter(
            models.Q(redirect_to__isnull=False)
            | models.Q(redirect_to_other__isnull=False)
        )
        .delete()
    )
    if del_objs[0]:
        logger.debug(
            "Delete {} WebApp which redirect to other application,deleted objects = {}".format(
                del_objs[0], del_objs[1]
            )
        )

    # delete the not-exist server which doesn't redirect to other webapp
    del_objs = (
        WebApp.objects.exclude(pk__in=serverids)
        .filter(redirect_to__isnull=True, redirect_to_other__isnull=True)
        .delete()
    )
    if del_objs[0]:
        logger.debug(
            "Delete {} WebApp which doesn't redirect to other application,deleted objects = {}".format(
                del_objs[0], del_objs[1]
            )
        )
...
def save_location_forward_servers(app_location, location_servers, created=False):
    if WebAppLocationServer.objects.filter(
        location=app_location, user_added=True
    ).exists():
        # user added some forward servers
        if not WebAppLocationServer.objects.filter(
            location=app_location, user_added=False
        ).exists():
    ...
    for location_server in location_servers:
        ...
        if Cluster.objects.filter(
            models.Q(name=hostname) | models.Q(ip=hostname)
        ).first():
        ...
        try:
            ...
            if other_name and (
                not location_server[0].other_names
                or other_name not in location_server[0].other_names
            ):
            ...
            location_server[0] = WebServer(
                name=hostname,
                category=hostcategory,
                other_names=[other_name] if other_name else None,
            )
    ...
    for server in list(WebAppLocationServer.objects.filter(location=app_location)):
        ...
        while index >= 0:
            if (
                location_servers[index][0] == server.server
                and location_servers[index][1] == server.port
            ):
        ...
        if not location_server and not server.user_added:
            ...
            server.delete()
            logger.debug(
                "Delete the WebAppLocationServer({1}) from location({0})".format(
                    server, app_location
                )
            )
    ...
    while index >= 0:
        ...
        if index in founded_index:
            # already created
            server = WebAppLocationServer.objects.get(
            ...
            if rancher_workload != server.rancher_workload:
                ...
                logger.debug(
                    "Update the WebAppLocationServer({1}) from location({0})".format(
                        app_location, server
                    )
                )
        else:
            ...
            server.save()
            logger.debug(
                "Create the WebAppLocationServer({1}) from location({0})".format(
                    app_location, server
                )
            )
        index -= 1
...
def parse_server(server, domains=None, system_envs=None):
    ...
    if created:
        logger.info(
            "Create the system alias({1}) for the server({0}) ".format(server, name)
        )
...
listen_re = re.compile(
    "^listen\s+(?P<host>[0-9a-zA-Z\-\_\.\*]+):(?P<port>[0-9]+)(\s+(?P<https>ssl))?(\s+|;|$)"
)
...
sso_auth_types = [
    ...
    (
        WebAppLocation.SSO_AUTH_DUAL,
        re.compile("\s+auth_request\s+/sso/auth_dual(\s+|;|\}|$)"),
    ),
    (
        WebAppLocation.SSO_AUTH_TO_DBCA,
        re.compile(
            "\s+(uwsgi_pass\s+|proxy_pass\s+https?:\/\/)authome_dbca(\s+|;|\}|$)"
        ),
    ),
    (
        WebAppLocation.SSO_AUTH_TO_DPAW,
        re.compile(
            "\s+(uwsgi_pass\s+|proxy_pass\s+https?:\/\/)authome_dpaw(\s+|;|\}|$)"
        ),
    ),
    (
        WebAppLocation.SSO_AUTH_TO_UAT,
        re.compile(
            "\s+(uwsgi_pass\s+|proxy_pass\s+https?:\/\/)authome_uat(\s+|;|\}|$)"
        ),
    ),
    (WebAppLocation.NO_SSO_AUTH, None),
]
location_re = re.compile(
    "(^|\s+)location\s+((?P<modifier>[\=\~\*\^]+)\s+)?(?P<location>[\S]+)\s*\{"
)
location_types = [
...
cors_re = re.compile(
    "\s+add_header\s+('|\")Access-Control-Allow-Origin('|\")\s+('|\")\$http_origin('|\")\s+always\s*;"
)
redirect_re = re.compile(
    "(^|\s+)redirect\s+(?P<target_server>[a-zA-Z0-9_\-\.]+)(?P<path>/[\S]*)?\s*;"
)

location_redirect_res = [
    re.compile(
        "(^|\s+)return\s+(?P<http_code>[0-9]+)\s+(?P<location>[\S]+)(\s+|;|\}|$)"
    ),
    re.compile(
        "(^|\s+)rewrite\s+(?P<src>[\S]+)\s+(?P<target>[\S]+)\s+(redirect|permanent)"
    ),
]
...
protocol_types = [
    (
        WebAppLocation.HTTP,
        re.compile(
            "(^|\s+)proxy_pass\s+http:\/\/(?P<server>[\$a-zA-Z0-9\_\-\.]+)(:(?P<port>[0-9]+))?(?P<path>/[\S]*)?(\s+|;|\}|$)"
        ),
    ),
    (
        WebAppLocation.UWSGI,
        re.compile(
            "(^|\s+)uwsgi_pass\s+(?P<server>[\$a-zA-Z0-9\_\-\.]+)(:(?P<port>[0-9]+))?(?P<path>/[\S]*)?(\s+|;|\}|$)"
        ),
    ),
    (
        WebAppLocation.HTTPS,
        re.compile(
            "(^|\s+)proxy_pass\s+https:\/\/(?P<server>[\$a-zA-Z0-9\_\-\.]+)(:(?P<port>[0-9]+))?(?P<path>/[\S]*)?(\s+|;|\?|$)"
        ),
    ),
]
...
def parse_server_config(server, server_config, upstream_servers):
    ...
    def _save_location(location_lines):
        if location_lines:
            ...
            if not m:
                raise Exception(
                    """Can't find the location from location configuration""
{}""".format(
                        configure_txt
                    )
                )
            ...
            location_type = next(
                (o[0] for o in location_types if o[1] == m.group("modifier")), None
            )
            if not location_type:
                raise Exception(
                    "Location modifier({}) Not Support".format(m.group("modifier"))
                )

            # check interal location
            internal_location = (
                True if internal_location_re.search(configure_txt) else False
            )
            ...
            if location_type == WebAppLocation.PREFIX_LOCATION and location == "/sso":
                ...
                if auth_type not in (
                    ...
                    raise Exception(
                        """Can't find the sso authentication type in configure
{}""".format(
                            configure_txt
                        )
                    )
            ...
            for protocol, protocol_re in protocol_types:
                ...
                if m:
                    ...
                    if m.group("port"):
                        ...
                        raise Exception(
                            """Port number is not found for protocol({}), server={}, location=
{} """.format(
                                forward_protocol, configure_txt
                            )
                        )
            ...
            if not forward_protocol and not redirect and not return_code and not refuse:
                logger.error(
                    """Both forward and redirect are not found .server={} location=
{}
""".format(
                        server, configure_txt
                    )
                )
            if forward_protocol and not forward_servers:
                raise Exception(
                    """Missing forward servers, parse location failed.server={}  location=
{}
""".format(
                        server, configure_txt
                    )
    ...
    for line in config_lines:
        ...
        if not s_line:
            ...
            if not m:
                raise Exception(
                    "Can't parse listen line({1}) for server({0})".format(
                        server, s_line
                    )
                )
            ...
            raise Exception(
                "Can't parse line({1}) for server({0})".format(server, line)
            )
