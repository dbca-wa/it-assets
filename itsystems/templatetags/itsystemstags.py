...
def renderButtons(context, content):
    ...
    if len(fields) > 0:
        ...
        clearAllButton = (
            """<button type="button" class="btn btn-outline-secondary m-1" onClick="clearFields("""
            + fields_string
...
def renderSelect(context, content, id, filter_name, list_name, nullable=False):
    ...
    if filter_name in context:
        ...
        if nullable and context[filter_name] is None:
...
def encodeURL(context, content):
    ...
    """
    format_kwargs = {"content": content, "context": context}
    url_string = ""
    if "order_by" in context:
