        ...
        for record in register:
            ...
            if record.business_service_owner:
                _process_contact(
                    ...
                    record=record,
                    field_name="Business Service Owner",
                    user=record.business_service_owner,
                    logger=logger,
                )
            else:
            ...
            if record.system_owner:
                _process_contact(
                    flagged_users=flagged_users, record=record, field_name="System Owner", user=record.system_owner, logger=logger
                )
            else:
                # Mandatory field, empty values must be flagged
            ...
            if record.technology_custodian:
                _process_contact(
                    flagged_users=flagged_users,
                    record=record,
                    field_name="Technology Custodian",
                    user=record.technology_custodian,
                    logger=logger,
                )
            if record.information_custodian:
                _process_contact(
                    flagged_users=flagged_users,
                    record=record,
                    field_name="Information Custodian",
                    user=record.information_custodian,
                    logger=logger,
                )
